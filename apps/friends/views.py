...
def userprofile(request, id):
    ...
    return render(request, 'friends/userprofile.html', context)

def otheruserprofile(request, id):
    context = {
        "user_trip": Trip.objects.filter(id = id)
    }
    return render(request, 'friends/otheruserprofile.html', context)


def remove(request, id):
    other = Trip.objects.removeuser(id, request.session['id'])
    if other[0] == True:
        messages.info(request, "Successful Joined A Friend!")
        return redirect('/success')
    else:
        for i in other[1]:
            messages.info(request, i)
