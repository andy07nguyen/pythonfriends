...
class UserManager(models.Manager):
    ...
    #ERROR CHECK:
        if no_error == True:
            newUser = User.objects.create(first_name = postData['fname'], last_name = postData['lname'], email = postData['email'], password = hashed, date = postData['date'])
...
class TripManager(models.Manager):
    ...
    #ERROR CHECK:
        if check == True:
            ...
            newTrip = Trip.objects.create(destination = postData['destination'], description = postData['description'], user_id = user)
...
#REMOVE USER CLASS:
    def removeuser(self, tripid, id):
        msg = []
        check = True

        if len(Trip.objects.filter(id = id).filter(joiner__id = tripid)) > 0:
            msg.append("You already removed this friend!")
            check = False
            return(False, msg)
        else:
            this_user = User.objects.get(id=id)
            this_trip = Trip.objects.get(id=tripid)
            join_trip = this_trip.joiner.remove(this_user)
            msg.append("Removed")
            check = True
            return(True, msg, join_trip)
